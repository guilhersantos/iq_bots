...
print('#:===============================================================:#')
print(f'Executando estratégia MILHAO MAIORIA')
...
def aguardar_horario_entrada(_iq: IQ_Option, _pariedade: str) -> list:
    ...
    while True:
        ...
        if (4.59 <= minutos <= 5) or minutos == 9.59:
            return _iq.get_candles(_pariedade, 60, 5, time.time())


def validar_estrategia(_candles: list) -> [bool, str]:
    print('>>>>>>>>>>>>>>> Validando estratégia', end='\r')
    print()
    cores = get_color_candle(_candles[0]) + get_color_candle(_candles[1]) + get_color_candle(
        _candles[2]) + get_color_candle(_candles[3]) + get_color_candle(_candles[4])
    if cores.count('D'):
        return False, "None"
